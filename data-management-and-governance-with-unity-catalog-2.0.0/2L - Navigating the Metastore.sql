...
-- MAGIC
-- MAGIC The relations found in this schema are documented <a href="https://docs.databricks.com/sql/language-manual/sql-ref-information-schema.html" target="_blank">here</a>. As a basic example, let's see all the tables in the catalog. Note that since we only specify two levels here, we're referring the default catalog selected earlier.
