...
-- MAGIC spark.sql(f"CREATE OR REPLACE TABLE movies_clone CLONE hive_metastore.{clean_username}.movies")
...
CREATE OR REPLACE VIEW movies_gold AS
SELECT _c0, title, year, length, budget, rating,
  CASE WHEN
    is_account_group_member('metastore_admins') THEN votes 
    ELSE 'REDACTED'
  END AS votes
FROM movies_clone
WHERE
  (is_account_group_member('metastore_admins') OR rating > 6.0)
ORDER BY rating;
...
GRANT SELECT ON VIEW movies_gold TO `account users`
