...
class DBAcademyHelper():
    ...
    def cleanup(self):
        spark.sql(f"DROP SCHEMA IF EXISTS hive_metastore.{self.schema} CASCADE")
        spark.sql(f"DROP CATALOG IF EXISTS {self.catalog} CASCADE")
        dbutils.fs.rm(f'/FileStore/tables/{self.schema}', recurse=True)
...
external_delta_table = f'dbfs:///FileStore/tables/{da.schema}'
